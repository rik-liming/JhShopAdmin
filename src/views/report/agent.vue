<template>
  <div class="app-container">
    <div class="filter-container tw-flex tw-flex-col tw-items-start md:tw-flex-row md:!tw-items-center">
      <el-date-picker
    ...
    >
      ...
      <el-table-column label="代理ID" align="center" width="150px" >
        <template v-slot="{row}">
          <span class="link-type" @click="handleShowDetail(row)">{{ formatIdDisplay(row?.user_id) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="代理邮箱" width="200px" align="center">
        <template v-slot="{row}">
